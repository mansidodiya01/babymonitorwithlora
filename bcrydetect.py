...
def detect_baby():
    ...
    while True:
        ...
        cv2.imwrite(original_image_path, cv2.resize(frame, (96, 96)))
        ...
        if best_box is not None:
            ...
            # Compress and save cropped image (≤ 5KB)
            quality = 85  # Start with high quality
            min_quality = 10  # Avoid excessive compression

            while quality >= min_quality:
                cv2.imwrite(cropped_image_path, cv2.resize(cropped_image, (96, 96)), [cv2.IMWRITE_JPEG_QUALITY, quality])
                size_kb = os.path.getsize(cropped_image_path) / 1024  # Convert bytes to KB

                if size_kb <= 5:
                    print(f"✅ Cropped image compressed and saved: {cropped_image_path} ({size_kb:.2f} KB)")
                    break  # Stop when size is under 5 KB
        ...
        time.sleep(max(0, 3 - elapsed_time))  # Ensure at least 3s interval
